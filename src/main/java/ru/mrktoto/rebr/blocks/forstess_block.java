...
public class forstess_block extends BlockTileEntity<TileEntityCounterPersonal> {

	public static final String[] metadata = new String[] {
			"forstess_block",
	        "forstess_block_meta_1", 
	        "forstess_block_meta_2", 
	        "forstess_block_meta_3", 
	        "forstess_block_meta_4", 
	    };
    ...
    public IIcon getIcon(int side, int metadata) {
        if (metadata >= 0 && metadata < forstess_block.metadata.length) {
	...
	public forstess_block(String name, Material material, float hardness, float resistanse, SoundType soundType) {
		...
		this.setBlockTextureName(rebr.MODID + ":" + "forstess_block/forstess_block");
	}
	...
	public float getPlayerRelativeBlockHardness(EntityPlayer player, World world, int x, int y, int z) {
		...
		if (!(player.inventory.getCurrentItem() == null) && (!(playerID == null)) && (!(entityID == null)) && (!world.isRemote)) {
			ItemStack stack = player.inventory.getCurrentItem();
		      ...
		      if (stack != null && ((stack.getItem() == BlocksRegistry.buildingmallet) || (stack.getItem() == BlocksRegistry.buildingmallet2) || (stack.getItem() == BlocksRegistry.buildingmallet3)) && entityID.equals(playerID))
		      {
		         return 1F;
