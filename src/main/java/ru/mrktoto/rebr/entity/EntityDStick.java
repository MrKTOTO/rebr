...
public class EntityDStick extends EntityThrowable
{
	protected String entityToSpawnName = "";
	
    public EntityDStick(World world)
    {
        super(world);
    }

    public EntityDStick(World world, EntityLivingBase thrower)
    {
        super(world, thrower);
        //this.setVelocity(this.motionX/4, this.motionY/4, this.motionZ/4);
        motionX /= 4;
        motionY /= 4;
        motionZ /= 4;
    }

    public EntityDStick(World world, double x, double y, double z)
    {
        super(world, x, y, z);
    }
        ...
        if(pos.typeOfHit == MovingObjectType.BLOCK && (!this.worldObj.isRemote)){
            ...
            if(tile instanceof TileEntityCounterPersonal){
            	...
            	if (block == BlocksRegistry.doorL1) {
            		...
            		if (((TileCodeDoor) tile).getCount() >= 3) {
                		worldObj.setBlock(pos.blockX, pos.blockY, pos.blockZ, Blocks.air);
    					float var1 = 0.5F;
    					worldObj.createExplosion(this, pos.blockX+0.5, pos.blockY+0.5, pos.blockZ+0.5, var1, true);
    				} else {
            		...
            		if (((TileCodeDoor) tile).getCount() >= 5) {
                		worldObj.setBlock(pos.blockX, pos.blockY, pos.blockZ, Blocks.air);
    					float var1 = 0.5F;
    					worldObj.createExplosion(this, pos.blockX+0.5, pos.blockY+0.5, pos.blockZ+0.5, var1, true);
    				} else {
